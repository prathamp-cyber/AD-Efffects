...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Security Hardening: Enforce maximum file size limit (10MB) to prevent Denial of Service (DoS)
    ...
    // Security Hardening: Whitelist file extensions to prevent upload of executable code/scripts (XSS/RCE)
    const allowedExtensions = ['.jpg', '.jpeg', '.png', '.webp', '.gif', '.svg'];
    if (!allowedExtensions.includes(extension.toLowerCase())) {
      return NextResponse.json({ 
        error: 'Invalid file type. Only JPG, JPEG, PNG, WEBP, GIF, and SVG images are allowed.' 
      }, { status: 400 });
    }
